#!/usr/bin/env python3
"""Coverage: Detox testIDs vs testIds.ts"""
import re, glob
...
used_ids = set()
js_files = sorted(glob.glob(f"{test_dir}/**/*.test.js", recursive=True))
print(f"Files scanned: {len(js_files)}")
for f in js_files:
    with open(f) as fh:
        content = fh.read()
    for m in re.findall(r"by\.id\(\s*'([^']+)'\s*\)", content):
        used_ids.add(m)

print(f"Unique test IDs used in Detox tests: {len(used_ids)}")
print()
