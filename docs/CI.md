...
| `Open in <App>?` system dialog blocks an iOS flow | Custom-scheme deep link confirmation                 | The conditional `tapOn 'Open'` in `_setup.yaml` dismisses it; copy that `runFlow.when.visible: 'Open'` guard into any flow that calls `openLink` |
